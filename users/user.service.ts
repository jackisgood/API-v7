  ...
  async updateStatus(patient_code,status) {
    ...
    return s;
     ...
     async  update3leads(param) {
      var start=new Date().getTime();
      var check_status = await this.userRepository.findOne({userId:param.ECG_3leadInfoViewModels_uploads[0].Patient_CodeID});
      ...
      if (check_status.Status) {
		...
		var data:any=[];
      		...
      		var reg1=param.ECG_3leadInfoViewModels_uploads[j].Diff_1.split(",");
      		var t=reg1[0];
      		t=parseInt(t);
      		var reg2=param.ECG_3leadInfoViewModels_uploads[j].Diff_2.split(",");
      		var reg3=param.ECG_3leadInfoViewModels_uploads[j].Diff_3.split(",");
      		var d1:any=[];
      		var d2:any=[];
      		var d3:any=[];
      		...
      		const query: any = {
      			where: {  Patient_CodeID : param.ECG_3leadInfoViewModels_uploads[j].Patient_CodeID },
      		};
		...
		//return ex;
			...
			var raw:any=[];
        		raw={
        			Data_Point_Amount: param.ECG_3leadInfoViewModels_uploads[j].Data_Point_Amount,
        			Date:param.ECG_3leadInfoViewModels_uploads[j].Date,
        			Ecg_time:t,
        			Current_time:Date.now(),
        			Diff_1:d1,
        			Diff_2:d2,
        			Diff_3:d3,
        			Patient_CodeID:param.ECG_3leadInfoViewModels_uploads[j].Patient_CodeID,
